...
def main():
    parser = argparse.ArgumentParser(
        description="基于 MediaPipe 骨架检测的智能视频剪辑工具"
    )
    parser.add_argument("inputs", nargs="+", help="输入视频路径")
    parser.add_argument("--output", "-o", default="clip_output.mp4",
                        help="输出视频路径")
    parser.add_argument("--duration", "-d", type=float, default=5.0,
                        help="单视频目标时长（秒），默认5秒")
    parser.add_argument("--total", "-t", type=float, default=None,
                        help="批量模式总时长（秒），默认=duration*视频数")
    ...
    if len(args.inputs) == 1:
        # 单视频模式
        clip.clip_video(args.inputs[0], args.output, target_duration=args.duration)
    else:
        # 批量模式
        total = args.total or (args.duration * len(args.inputs))
        clip.batch_clip(args.inputs, args.output, total_duration=total)
