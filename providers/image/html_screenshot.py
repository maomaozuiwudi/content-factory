...
通过 HTML+CSS 精致排版，Playwright 截图生成封面/卡片/拼接图。
替代纯 Pillow 方案，解决设计差的问题。
"""
...
def _screenshot(html: str, output_path: str, full_page: bool = False):
...
class HtmlScreenshotImage:
    """HTML+Playwright 截图的生图 Provider"""
    ...
    def generate_cover(self, title: str = "", subtitle: str = "",
        ...
        if tags:
            tag_items = "".join(
                f'<span class="tag">#{html.escape(t)}</span>' for t in tags[:5]
            )
        ...
        if subtitle_lines:
            sub_html = "".join(
                f'<div class="sub-line">{html.escape(line)}</div>' for line in subtitle_lines
            )
...
<body>
  ...
  {_WATERMARK_HTML}
    ...
    def generate_card(self, title: str = "", subtitle: str = "",
        ...
        title_html = "".join(
            f'<div class="card-title-line">{html.escape(line)}</div>' for line in title_lines
        )
        ...
        if bullet_points:
            items = "".join(
                f'<li>🔹 {html.escape(pt)}</li>' for pt in bullet_points[:8]
            )
...
body {{
  ...
  background:linear-gradient(135deg, {LIGHT_BG_1} 0%, {LIGHT_BG_2} 100%);
...
<body>
  <div class="card">
    ...
    def stitch_images(self, image_paths: list, layout: str = "grid",
                      output_path: str = None) -> str:
        """多图拼接 — vertical 竖排 + 每张图上方文字标注 + 分割线

        layout: 'vertical' | 'grid' — 默认 vertical 竖排
        """
        if not image_paths:
        ...
        if output_path is None:
            output_path = os.path.join(OUTPUT_DIR,
                                       f"stitch_{layout}_{len(image_paths)}pics.png")
        ...
        def _img_to_b64(path):
            try:
                ...
                print(f"[⚠️ 拼接] 无法加载 {path}: {e}")
                return None

        if layout == "grid":
            return self._stitch_grid(image_paths, output_path)
        ...
        for i, img_path in enumerate(image_paths):
            ...
            if b64 is None:
                continue
            label = Path(img_path).stem[:30]
            items_html += f"""
<div class="stitch-item">
  <div class="stitch-label">图{i+1}：{label}</div>
  <img class="stitch-img" src="{b64}" />
</div>"""
...
{_BASE_RESET}
body {{
  width:1080px; padding:30px 20px;
  background:#FAFAFA;
}}
.stitch-item {{
...
.stitch-label {{
  ...
  padding:14px 24px; letter-spacing:1px;
  border-bottom:1px solid #eee;
}}
.stitch-img {{
  display:block; width:1040px; height:auto;
  margin:0 auto;
}}
.divider {{
  width:80%; height:2px; margin:10px auto 30px;
...
{items_html}
    ...
    def _stitch_grid(self, image_paths: list, output_path: str) -> str:
        """grid 布局拼接 — 2列网格"""
        import base64
        from pathlib import Path
        ...
        for i, img_path in enumerate(image_paths):
            ...
            if b64 is None:
                continue
            label = Path(img_path).stem[:20]
            items += f"""
<div class="grid-cell">
  <div class="grid-label">图{i+1}</div>
  <img class="grid-img" src="{b64}" />
</div>"""
...
{items}
        ...
        _screenshot(html, output_path)
        print(f"[🖼️ 生图] 拼接图(Grid)已保存: {output_path}")
        return output_path
